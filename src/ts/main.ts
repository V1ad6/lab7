...
function binaryReverseSum(reverseBin1: string, reverseBin2: string): string {
  let result: string = '';
  let curr: number = 0;
...
function calcResult(term1: number, term2: number): string[] | string {
  ...
  const [straigthCode1, reverseCode1]: string[] = t1;
  const [straigthCode2, reverseCode2]: string[] = t2;

  const reverseCodeSum: string = binaryReverseSum(reverseCode1, reverseCode2);
